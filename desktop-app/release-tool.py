...
class Check(Command):
    ...
    def check_app_stream_info(version, cwd=None):
        appstream = Path('share/linux/org.they-know-your-passwords.TheyKnowYourPasswords.appdata.xml')
        if cwd:
...
class Build(Command):
    ...
    def _build_linux_appimage(self, version, src_dir, output_dir, app_dir, build_dir, install_prefix,
        ...
        icon_file = install_prefix / 'share/icons/hicolor/256x256/apps/they-know-your-passwords.png'
...
class I18N(Command):
    ...
    def run_lupdate(self, src_dir, build_dir=None, commit=False, lupdate_args=None):
        ...
        _run(['lupdate', '-no-ui-lines', '-disable-heuristic', 'similartext', '-locations', 'none',
              ...
              '-ts', str(Path(f'share/translations/they-know-your-passwords_en.ts')), *(lupdate_args or [])],
             cwd=src_dir, path=path, capture_output=False)
        logger.info('Translation source files updated.')
        if commit:
            _git_commit_files([f'share/translations/they-know-your-passwords_en.ts'],
