...
* **KDE Plasma 6.6+** — Supported since 5.27: reconfiguration before 6.6 is done via _System Settings_ -> _Shortcuts_ -> _KeePassXC_; before 6.4 have a bug that brings up the configuration dialog on every start.
* **GNOME 51+** — Supported since 48: reconfiguration from 48 to 50 is done via _GNOME Settings_ -> _Apps_ -> _KeePassXC_.
